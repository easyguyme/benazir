...
class Login{
    ...
    private function are_credentials_right(){
        $select_user_and_pass_query = $this->conn->prepare("SELECT username, password FROM registration WHERE email = :username AND password = :password");
    ...
    private function is_user_allowed_access(){
        $is_user_banned_query = $this->conn->prepare("SELECT is_account_banned, account_unban_date FROM registration WHERE email = :username");
